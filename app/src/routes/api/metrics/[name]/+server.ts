...
import {
  ...
  TIME_WINDOWS,
  type TimeWindow,
} from "$lib/server/prometheus/queries";
...
export const GET: RequestHandler = async ({ params, url }) => {
  ...
  try {
    ...
    const [cpuSeries, memorySeries, jobsSeries, totalJobs, failedJobs, jpm] =
      await Promise.all([
        prometheus.rangeQuery(
          QUERIES.cpuUsage(params.name),
          start,
          now,
          config.step,
        ),
        prometheus.rangeQuery(
          QUERIES.memoryUsage(params.name),
          start,
          now,
          config.step,
        ),
        prometheus.rangeQuery(
          QUERIES.jobsPerMinute(params.name),
          start,
          now,
          config.step,
        ),
