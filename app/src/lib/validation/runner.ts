...
export function validateRunnerConfig(config: Partial<RunnerConfig>): ValidationError[] {
	...
	if (config.hpa) {
		...
		if (config.hpa.cpu_target < 1 || config.hpa.cpu_target > 100) {
			errors.push({ field: 'hpa.cpu_target', message: 'Must be between 1 and 100' });
		}
		if (config.hpa.memory_target < 1 || config.hpa.memory_target > 100) {
			errors.push({ field: 'hpa.memory_target', message: 'Must be between 1 and 100' });
		}
	}

	// Validate resource strings
	for (const key of ['cpu_request', 'cpu_limit', 'memory_request', 'memory_limit'] as const) {
		if (config.manager_resources?.[key] && !validateResourceString(config.manager_resources[key])) {
			errors.push({ field: `manager_resources.${key}`, message: 'Invalid K8s resource format' });
		}
		if (config.job_resources?.[key] && !validateResourceString(config.job_resources[key])) {
			errors.push({ field: `job_resources.${key}`, message: 'Invalid K8s resource format' });
		}
	}
