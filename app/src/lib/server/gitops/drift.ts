...
export function detectDrift(
	...
	for (const hpa of hpas) {
		...
		if (configMin !== undefined && typeof configMin === 'number' && hpa.min_replicas !== configMin) {
			drifts.push({
				runner: hpa.name,
		...
		if (configMax !== undefined && typeof configMax === 'number' && hpa.max_replicas !== configMax) {
			drifts.push({
				runner: hpa.name,
	...
	for (const runner of deployedRunners) {
		...
		if (shouldDeploy === true) {
			...
			const found = hpas.some((h) => h.name === hpaName || h.name.startsWith(hpaName));
