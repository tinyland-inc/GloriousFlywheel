 ...
 */
export async function readFile(path: string, ref: string = 'main'): Promise<string> {
 ...
 */
export async function createBranch(branchName: string, ref: string = 'main'): Promise<void> {
...
export async function commitFile(
	...
	const encodedPath = encodeURIComponent(path);
	await gitlab.request(`/projects/${PROJECT_ID}/repository/files/${encodedPath}`, {
		method: 'PUT',
		body: {
			branch,
			content,
