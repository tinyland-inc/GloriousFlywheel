...
export interface TfVarsLine {
	type: 'assignment' | 'comment' | 'blank' | 'map-start' | 'map-entry' | 'map-end';
	raw: string;
	key?: string;
...
export function parseTfVars(content: string): TfVarsDocument {
	...
	const values: Record<string, string | number | boolean | Record<string, string>> = {};
...
function formatValue(value: string | number | boolean | Record<string, string>): string {
