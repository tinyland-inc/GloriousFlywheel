...
export class PrometheusClient {
	...
	async rangeQuery(
		...
		const response = await this.fetch<RangeResult>(`/api/v1/query_range?${params}`);
		return response.data.result;
	}
	...
	private async fetch<T>(path: string): Promise<PrometheusResponse<T>> {
		...
		if (!response.ok) {
			throw new PrometheusError(response.status, `Prometheus query failed: ${response.status}`);
		}
